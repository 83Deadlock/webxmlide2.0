...
export default {
    ...
    computed: mapState(["xsd_code"]),
    methods: mapMutations(["updateXSDCode"]),
    mounted() {
        const vm = this;
        ...
        const editorPromise = new Promise((resolve) => {
            ...
            editor.setValue(this.xsd_code);
        })

        editorPromise.then((editor) => {
            editor.on('change', function () {
                const newContent = editor.getValue();
                vm.$store.commit('updateXSDCode', newContent);
            });            
        }).catch((error) => {
            console.error(error);
        });

        this.$watch('xsdCode', (newVal) => {
            if(editorPromise) {
                editorPromise.then((editor) => {
